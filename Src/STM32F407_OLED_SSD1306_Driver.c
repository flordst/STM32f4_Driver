...
static void GPIO_Config(void)
{
	...
	/* Init Typdef */
  GPIO_Handle_t myPinInit;
  myPinInit.pGPIOx=GPIOB;

  /* I2C Pin Config */
  myPinInit.GPIO_Config.GPIO_PinNumber = GPIO_PIN_6 | GPIO_PIN_7 ;
  myPinInit.GPIO_Config.GPIO_PinMode = GPIO_ALTFN;
  myPinInit.GPIO_Config.GPIO_PinOPType = GPIO_OP_OD;
  myPinInit.GPIO_Config.GPIO_PinSpeed = GPIO_SPEED_HIGH;
  myPinInit.GPIO_Config.GPIO_PinAltFuncMode = 4;
  GPIO_Init( &myPinInit);
...
static void I2C_Config(void)
{
	...
	myI2Chandle.pI2Cx = I2C1;
	myI2Chandle.I2C_Config.ACKControl = ENABLE;
	myI2Chandle.I2C_Config.SCLSpeed = SCL_SPEED_FM4K;
	myI2Chandle.I2C_Config.DeviceAddress=STM32F4_I2C_ADDRESS;
	myI2Chandle.I2C_Config.FMDutyCycle=FMDUTY_2;
	I2C_Init(&myI2Chandle);
}
...
void OLED_SSD1306_Send_Command(uint8_t cmd)
{
	...
	/* Transmit the command */
		I2C_MasterTransmit(& myI2Chandle,command_tx,2,OLED_I2C_ADDRESS, 0);
}
...
void OLED_SSD1306_Send_Data(uint8_t data)
{
  /* control byte to send commands "Co=0 D/C=1 0 0 0 0 0 0 ---> 0x00" 
	...
	/* Transmit the data */
	I2C_MasterTransmit(& myI2Chandle,data_tx,2,OLED_I2C_ADDRESS, 0);
}
  ...
  for (m = 0; m < 8 ; m++)
	{
		...
		/* Trasnmit the appropriate data in the OLED_buffer */
		 I2C_MasterTransmit(& myI2Chandle,temp,OLED_WIDTH+1,OLED_I2C_ADDRESS,100);
